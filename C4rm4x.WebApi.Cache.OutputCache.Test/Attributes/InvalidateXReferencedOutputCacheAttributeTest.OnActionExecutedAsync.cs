...
namespace C4rm4x.WebApi.Cache.OutputCache.Test
{
    ...
    {
        [TestClass]
        public class InvalidateXReferencedOutputCacheAttributeOnActionExecutedAsyncTest
        {
            ...
            [TestMethod, UnitTest]
            public async Task OnActionExecutedAsync_Checks_Whether_Or_Not_The_Key_For_The_Given_ControllerType_And_Action_Name_Is_In_The_Cache_When_Response_Was_Successfully_Processed()
            {
                var cache = GetCache();
                ...
                Mock.Get(cache)
                    .Verify(c => c.ExistsAsync(GetKey(typeof(TestController), ActionName)), Times.Once());
            }

            [TestMethod, UnitTest]
            public async Task OnActionExecutedAsync_Does_Not_Try_To_Remove_The_Entry_From_The_Cache_When_The_Key_Is_Not_Stored()
            {
                var cache = GetCache();
            ...
            [TestMethod, UnitTest]
            public async Task OnActionExecutedAsync_Tries_To_Remove_The_Entry_From_The_Cache_When_The_Key_Is_Stored()
            {
                var cache = GetCache();
                ...
                Mock.Get(cache)
                    .Verify(c => c.RemoveAsync(GetKey(typeof(TestController), ActionName)), Times.Once());
            }
