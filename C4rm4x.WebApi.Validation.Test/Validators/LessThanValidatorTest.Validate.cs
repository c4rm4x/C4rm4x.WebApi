...
namespace C4rm4x.WebApi.Validation.Test.Validators
{
    ...
    {
        [TestClass]
        public class LessThanValidatorValidateTest :
            AbstractValidatorTest<LessThanValidator>
        {
            private const int ValueToCompare = 5;

            [TestMethod, UnitTest]
            public void Validate_Returns_No_ValidationErrors_When_Value_Is_Less_Than_Expected()
            {
            ...
            [TestMethod, UnitTest]
            public void Validate_Returns_A_ValidationError_When_Value_Is_Greater_Than_Expected()
            {
                var value = ValueToCompare + GetRand();
                var errors = Validate(value);
