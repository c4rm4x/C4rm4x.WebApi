...
namespace C4rm4x.WebApi.Framework.Test.RequestHandling.Results
{
    ...
    {
        [TestClass]
        public class ConflictResultExecuteAsyncTest
        {
            [TestMethod, UnitTest]
            public void ExecuteAsync_Returns_Conflict_Response()
            {
                Assert.AreEqual(
            ...
            private static ConflictResult CreateSubjectUnderTest(string reason = null)
            {
                return new ConflictResult(reason ?? ObjectMother.Create<string>());
            }
